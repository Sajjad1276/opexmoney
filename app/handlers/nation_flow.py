...
def _nations_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
...
async def open_nations(message: Message) -> None:
    ...
    if user is None:
        await message.answer("⚠️ اول با «🎮 شروع بازی» وارد OPEX شو.")
        return

    await message.answer(
        "🌍 <b>ملت‌ها</b>\n\n"
        "از این بخش می‌تونی به فرآیندهای مربوط به ملت‌ها دسترسی داشته باشی.\n\n"
        "🏛 اگر آماده‌ای، می‌تونی ملت جدید تأسیس کنی.",
        reply_markup=_nations_menu_keyboard(),
        parse_mode="HTML",
    )
...
@nation_router.message(F.chat.type.in_({"group", "supergroup"}))
...
@nation_router.callback_query(F.data == "start_founder")
async def start_founder(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if user is None or holding is None:
        if call.message:
            await call.message.answer("⚠️ اول با «🎮 شروع بازی» ثبت‌نامت رو کامل کن.")
        await call.answer()
        return
...
@nation_router.message(FounderStates.SET_NATION_NAME, F.text)
...
@nation_router.message(FounderStates.SET_CURRENCY_CODE, F.text)
...
@nation_router.callback_query(F.data == "cancel_founder")
