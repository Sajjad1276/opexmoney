...
from app.keyboards.inline import cancel_keyboard, first_trade_keyboard, nation_selection_keyboard, suggested_name_keyboard, welcome_keyboard
...
USERNAME_CAPTION = rtl_text("""<b>👤 هویت معامله‌گرت</b>

اسم تو از این لحظه روی تابلوی اقتصاد OPEX دیده می‌شه.
یک نام ۳ تا ۲۰ کاراکتری بساز؛ فارسی، انگلیسی، عدد و خط تیره مجاز است.
<u>این نام را بعداً از تنظیمات هم می‌توانی تغییر بدهی.</u>""")

INVALID_NAME = rtl_text("""<b>🔴 این نام وارد بازی نمی‌شود</b>

نام باید بین ۳ تا ۲۰ کاراکتر باشد.
<u>فقط فارسی، انگلیسی، عدد و خط تیره مجاز است.</u>""")

BLOCKED_NAME = rtl_text("""<b>🔴 این نام قابل استفاده نیست</b>

این نام با قوانین نام‌گذاری OPEX سازگار نیست.
<u>یک هویت معاملاتی دیگر انتخاب کن.</u>""")

CANCEL_TEXT = rtl_text("""<b>ثبت‌نام متوقف شد.</b>

هر وقت آماده بودی، با /start دوباره وارد شو و هویتت را بساز.""")

NAME_ACCEPTED_TEXT = rtl_text("""✅ <b>هویت «{username}» ثبت شد.</b>

حالا وقت انتخاب ملت است.""")

NO_NATION_TEXT = rtl_text("""🌍 <b>نقشه هنوز یک ملت کم دارد.</b>

هیچ ملت فعالی برای ورود وجود ندارد.
<u>از بخش ملت‌ها می‌توانی نخستین ملت را تأسیس کنی.</u>""")
...
async def _begin_registration(
    ...
    *,
    ...
    if suggested:
        ...
        text = (
            "👤 <b>هویت معامله‌گرت آماده است</b>\n\n"
            f"می‌توانیم با نام «{html.escape(suggested)}» وارد شویم.\n"
            "این نام بعداً از تنظیمات قابل تغییر است.\n\n"
            "یا یک نام ۳ تا ۲۰ کاراکتری کاملاً برای خودت بساز."
        )
        markup = suggested_name_keyboard(suggested)
    else:
        text = """
👤 <b>هویت معامله‌گرت را بساز</b>

یک نام ۳ تا ۲۰ کاراکتری انتخاب کن.
این نام روی بازار، رتبه‌بندی و رویدادهای بازی شناخته می‌شود.

فارسی، انگلیسی، عدد و خط تیره مجاز است.
"""
...
async def cancel_onboarding_panel(
    ...
    if registered and user is not None and call.message is not None:
        ...
        await call.message.answer(
            rtl_text(
                "👋 <b>دروازه OPEX هنوز باز است.</b>\n\n"
                "برای ورود، «شروع بازی» را بزن."
            ),
