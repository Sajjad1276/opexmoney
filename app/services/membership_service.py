...
async def sync_telegram_membership(
    ...
    *,
    ...
    # A newly created active projection represents a real transition.
    # Do not treat its initial is_active=True value as a previous state.
    ...
    if active_now:
        ...
        user_registered = user is not None and bool((user.username or "").strip())
        if user is not None:
            await reconcile_user_wallet(session, user.user_id)
