...
async def detect_revision(conn: asyncpg.Connection) -> str | None:
    ...
    if await column_exists(conn, "nation_wars", "ends_at"):
        highest = "0014_nation_war_flow"

    if await table_exists(conn, "nation_founding_drafts"):
        highest = "0015_nation_founding_drafts"
    ...
    ai_columns = {
        ...
        for row in await conn.fetch("""
            ...
            WHERE table_schema = 'public'
              AND (
                  (table_name = 'nations' AND column_name = 'is_ai')
                  OR (table_name = 'users' AND column_name IN ('is_ai', 'ai_strategy'))
              )
        """)
    }
    if ai_columns == {"is_ai", "ai_strategy"} and await column_exists(
        conn, "nations", "is_ai"
    ):
        highest = "0016_ai_world"

    if await table_exists(conn, "nation_telegram_members"):
        highest = "0017_telegram_membership"

    if await table_exists(conn, "nation_economic_events"):
        highest = "0018_economic_events"
